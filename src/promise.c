...
promise_t* promise_alloc(UV refcount) {
	promise_t* result = PerlMemShared_calloc(1, sizeof(promise_t));
...
static void promise_set(promise_t* promise, SV* value, enum value_type type) {
	MUTEX_LOCK(&promise->mutex);
